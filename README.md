...
面向西安电子科技大学学生的“今天吃什么”推荐应用。学生选择校区、餐别、就餐方式、地点、主类别、口味倾向，以及素食/清真等需求后，系统会从西电后勤公众号正文菜单里推荐合适档口与菜品。综合楼、家属区和外卖商家只通过学生提交与后台审核后进入推荐。

## 当前功能

- 学生端实时推荐界面：餐别、校区、堂食/外卖偏好、堂食地点多选、主类别单选、口味倾向、加分偏好。
- 南校区老综、新综、家属区商家支持学生共建提交；同一商家可以标记为同时支持堂食和外卖。
- 推荐算法：综合校区、供应时段、地点、热度、口味匹配、避雷项、素食/清真硬约束和轮换随机性打分。
...
- 局域网前台：`http://192.168.3.89:8080/`
- 管理后台：`http://192.168.3.89:8080/admin`
...
项目已经接入公开的西电后勤公众号文章作为真实食堂来源索引，并优先使用公众号正文直接提取的窗口与菜品：
...
- `src/data/xduWechatTextCanteens.generated.ts`：前端食堂数据，包含正文菜单候选，不展示历史价格。
- `src/data/xduOfficialCanteens.generated.ts`：保留 OCR/人工复核数据，供后续复核或对照使用。
